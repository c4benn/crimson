...
self.addEventListener(
  ...
  async (e) => {
    ...
    if (e.data.name == "spring") {
      ...
      const output = await mapFromAndTo();
      const $css = {};
      ...
      Object.keys($css).forEach((x, i, a) => {
        ...
        if (i + 1 == a.length) {
          ...
          const keyFrames = buildFrames();
          const duration = `${((output[0].frames + 1) / 60) * 1000}ms`;
          const classRule = e.data.classRule;
          const $output = {
            name: "spring",
            id: e.data.id,
            data: output,
    ...
    if (e.data.name == "color") {
      ...
      function convertColors(color) {
        ...
        if (/^hsla|^hsl/.test(color)) {
          ...
          return `rgba(${hslToRgb(args)})`;
        }

        return color;
      }

      const value = convertColors(e.data.color);
